...
_char_cache: dict[str, dict] = {}  # session_id:region -> {data, expires_at}
def _get_char_cache_ttl() -> int:
    from database import get_config
    try:
        return max(1, int(get_config("app.char_cache_ttl_minutes", "60"))) * 60
    except Exception:
        return 3600
...
async def list_characters(session: str, region: str = _DEFAULT_REGION):
    ...
    if resp.status_code == 401:
        raise HTTPException(
            status_code=401,
            detail="Sesja Battle.net wygasła — zaloguj się ponownie."
        )
    if resp.status_code == 403:
        raise HTTPException(
            status_code=403,
            detail="Brak dostępu do profilu Battle.net."
        )
    if resp.status_code != 200:
        raise HTTPException(
            status_code=502,
            detail=f"Błąd Blizzard API: {resp.status_code}"
        )
    ...
    _char_cache[cache_key] = {
        "data": chars,
        "expires_at": _time.time() + _get_char_cache_ttl(),
    }

    return chars


@router.post("/api/characters/refresh")
async def refresh_characters(session: str, region: str = _DEFAULT_REGION):
    """Wymusza odświeżenie listy postaci — czyści cache dla tej sesji."""
    cache_key = f"{session}:{region.lower()}"
    _char_cache.pop(cache_key, None)
    return await list_characters(session, region)
