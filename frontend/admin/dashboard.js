...
function renderDashboardCharts(data) {
  ...
  if (trend.length) {
    Plotly.newPlot('chart-trend', [{
      ...
      hovertemplate: `%{x}<br><b>%{y} ${adminT('admin.chart.simulations')}</b><extra></extra>`,
    }], {
      ...PLOTLY_LAYOUT_BASE,
    ...
    document.getElementById('chart-trend').innerHTML = noData;
  }
  ...
  if (classes.length) {
    ...
    Plotly.newPlot('chart-classes', [{
      ...
      y: sorted.map(c => c.character_class || adminT('common.unknown')),
      type: 'bar', orientation: 'h',
      marker: { color: sorted.map(c => CLASS_COLORS[c.character_class] || '#555') },
      hovertemplate: `<b>%{y}</b><br>%{x} ${adminT('admin.chart.simulations')}<extra></extra>`,
    }], {
      ...PLOTLY_LAYOUT_BASE,
    ...
    document.getElementById('chart-classes').innerHTML = noData;
  }

  const fs = data.fight_style_distribution || [];
  if (fs.length) {
    Plotly.newPlot('chart-fightstyle', [{
      labels: fs.map(f => f.fight_style || adminT('common.unknown')),
      values: fs.map(f => f.count),
      type: 'pie',
      marker: { colors: ['#f4a01c','#3FC7EB','#AAD372','#A330C9','#C41E3A','#8788EE'] },
      textinfo: 'percent+label',
      hovertemplate: `<b>%{label}</b><br>%{value} ${adminT('admin.chart.simulations')} (%{percent})<extra></extra>`,
    ...
    document.getElementById('chart-fightstyle').innerHTML = noData;
  }

  const top10 = data.top_dps || [];
  const container = document.getElementById('chart-top10');
  if (!top10.length) {
    container.innerHTML = `<p style="color:#555;text-align:center;padding:2rem 0">${adminT('admin.chart.no_data')}</p>`;
    return;
  }
  container.innerHTML = `
    <table style="width:100%;border-collapse:collapse">
      <thead>
        <tr style="color:#666;font-size:0.75rem;text-transform:uppercase;border-bottom:1px solid #2a2a2a">
          ...
          <th style="text-align:left;padding:0.3rem 0.4rem">${adminT('admin.chart.character')}</th>
          <th style="text-align:left;padding:0.3rem 0.4rem">${adminT('admin.chart.class_spec')}</th>
