  ...
  const result = {
    ...
    async initSettings() {
      ...
      try {
        ...
        if (res.status === 404) {
          this.error   = Alpine.store('i18n').t('errors.not_found');
          this.loading = false;
          return;
        }
        if (!res.ok) {
        ...
        this.error = Alpine.store('i18n').t('errors.network');
      } finally {
        this.loading = false;
      }
    },
    ...
    async save() {
      ...
      if (name && !realm) {
        this.saveMsg   = Alpine.store('i18n').t('settings.realm_required');
      ...
      try {
        ...
        if (res.status === 401) {
          this.saveMsg   = Alpine.store('i18n').t('errors.session_expired');
          this.saveMsgOk = false;
          return;
        }
        if (!res.ok) {
        ...
        this.saveMsg   = Alpine.store('i18n').t('errors.network');
        this.saveMsgOk = false;
      } finally {
        this.saving = false;
      }
